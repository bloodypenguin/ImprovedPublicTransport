...
namespace ImprovedPublicTransport
{
  public class PublicTransportStopWorldInfoPanel : UIPanel
  {
    ...
    private void SetupPanel()
    {
      ...
      UILabel uiLabel6;
      int num19 = 1;
      PublicTransportStopWorldInfoPanel.CreateStatisticRow((UIComponent) uiPanel5, out uiLabel3, out uiLabel4, out uiLabel5, out uiLabel6, num19 != 0);
      ...
      int num20 = 0;
      PublicTransportStopWorldInfoPanel.CreateStatisticRow((UIComponent) uiPanel5, out uiLabel3, out this.m_passengersInCurrent, out this.m_passengersInLast, out this.m_passengersInAverage, num20 != 0);
      ...
      int num21 = 0;
      PublicTransportStopWorldInfoPanel.CreateStatisticRow((UIComponent) uiPanel5, out uiLabel3, out this.m_passengersOutCurrent, out this.m_passengersOutLast, out this.m_passengersOutAverage, num21 != 0);
      ...
      int num22 = 0;
      PublicTransportStopWorldInfoPanel.CreateStatisticRow((UIComponent) uiPanel5, out uiLabel3, out this.m_passengersTotalCurrent, out this.m_passengersTotalLast, out this.m_passengersTotalAverage, num22 != 0);
    ...
    {
      this.ProcessNodes((System.Action<ushort>) (nodeID => Singleton<InstanceManager>.instance.SetName(new InstanceID()
      {
        NetNode = nodeID
    ...
    {
      NetManagerMod.m_cachedNodeData[(int) this.m_InstanceID.NetNode].Unbunching = !NetManagerMod.m_cachedNodeData[(int) this.m_InstanceID.NetNode].Unbunching;
    }

    private void OnUpdateCloseStopsClick(UIComponent component, UIMouseEventParameter eventParam)
    {
      this.ProcessNodes((System.Action<ushort>) (nodeID => NetManagerMod.m_cachedNodeData[(int) nodeID].Unbunching = NetManagerMod.m_cachedNodeData[(int) this.m_InstanceID.NetNode].Unbunching));
    }
    ...
    private void OnModifyLineClick(UIComponent component, UIMouseEventParameter eventParam)
    {
      if (this.m_InstanceID.Type != InstanceType.NetNode || (int) this.m_InstanceID.NetNode == 0)
        return;
      ushort transportLine = Singleton<NetManager>.instance.m_nodes.m_buffer[(int) this.m_InstanceID.NetNode].m_transportLine;
    ...
    private void OnDeleteStopClick(UIComponent component, UIMouseEventParameter eventParam)
    {
      if (this.m_InstanceID.Type != InstanceType.NetNode || (int) this.m_InstanceID.NetNode == 0)
        return;
      ushort transportLine = Singleton<NetManager>.instance.m_nodes.m_buffer[(int) this.m_InstanceID.NetNode].m_transportLine;
      if (!Singleton<TransportManager>.instance.m_lines.m_buffer[(int) transportLine].RemoveStop(transportLine, this.m_StopIndex))
        return;
      this.Hide();
    }
    ...
    private void ChangeInstanceID(InstanceID oldID, InstanceID newID)
    {
      ...
      ToolsModifierControl.cameraController.SetTarget(newID, ToolsModifierControl.cameraController.transform.position, Input.GetKey(KeyCode.LeftShift) | Input.GetKey(KeyCode.RightShift));
    }
    ...
    private void UpdatePosition()
    {
      if ((UnityEngine.Object) this.m_CameraTransform != (UnityEngine.Object) null)
      {
        ...
        Vector3 vector3_1 = Camera.main.WorldToScreenPoint(position) * Mathf.Sign(Vector3.Dot(position - this.m_CameraTransform.position, this.m_CameraTransform.forward));
        Vector2 vector2 = !((UnityEngine.Object) this.m_FullscreenContainer != (UnityEngine.Object) null) ? this.m_uiView.GetScreenResolution() : this.m_FullscreenContainer.size;
        ...
        Vector3 vector3_3 = (Vector3) (this.m_uiView.ScreenPointToGUI((Vector2) vector3_2) + new Vector2(transform.x, transform.y));
      ...
      this.m_SuggestedNames.ListPosition = this.m_StopName.absolutePosition + new Vector3(0.0f, this.m_StopName.height);
    }
    ...
    private ushort[] FindBuildings(Vector3 position)
    {
      ...
      foreach (ItemClass.Service service in Enum.GetValues(typeof (ItemClass.Service)))
      {
        ...
        {
          ushort building = instance.FindBuilding(position, 100f, service, subService, Building.Flags.Active, Building.Flags.Untouchable);
          if ((int) building != 0)
            source.Add(building);
        }
      }
