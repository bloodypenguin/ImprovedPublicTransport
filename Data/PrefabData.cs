...
namespace ImprovedPublicTransport2.Data
{
  public class PrefabData
  {
      ...
      get
      {
        ...
        switch (subService)
        {
          ...
          case ItemClass.SubService.PublicTransportTrolleybus:
            ...
            {
              MaintenanceCost = CalculateMaintenanceCost(Info, TotalCapacity, CarCount);
            }
    ...
    public static int CalculateMaintenanceCost(VehicleInfo vehicleInfo, int totalCapacity, int carCount)
    {
      var service = vehicleInfo.GetService();
      var subService = vehicleInfo.GetSubService();
      var level = vehicleInfo.GetClassLevel();
      
      float num = totalCapacity / (float)carCount /
                  GameDefault.GetCapacity(service, subService, level, vehicleInfo.m_vehicleType);
      return Mathf.RoundToInt(VanillaMaintenanceQuery.Query(service, subService, level, vehicleInfo.m_vehicleAI) * 16 * num);
    }
